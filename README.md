...
순서로 “어디서 온 증거이며, 무엇을 아직 모르는가”를 확인합니다. Catalog는
**Agent 365 agent-package와 Foundry 합성 검증 레코드**를 구분합니다. 수집할 때마다
변할 수 있는 수량은 [시각이 있는 현재 관측](docs/current-status.md#connector-ledger)을
참고하고 시연 화면의 값과 대조합니다. 이 수량은 실행 중인 production agent 수가
아니며 운영 목록은 개인별 사용 권한도 부여하지 않습니다.
...
| **Agent 365**                      | 읽기 전용 배포 관리 커넥터와 영속화된 원본 상태 구현                                                                          | **ready + complete:** 수량과 agent-package / extension-package 분류는 [현재 관측](docs/current-status.md#connector-ledger) 참조                                                                                                                                                                                                                           | 카탈로그 수집 완료이며 실행·설치·개인 사용 권한의 증거는 아님                                                     |
...
| **런타임 증거**                    | 제한된 Azure Monitor OTel 쿼리, 드리프트·신뢰성·실측 토큰·비용 분석 구현                                                      | `otel:primary`는 `degraded / not-queried`; 이번 수집의 적격·조회 에이전트와 런타임 증거는 0개                                                                                                                                                                                                                                                             | 실제 외부 실행 경계의 계측과 적격 데이터 필요. 과거 쿼리 접근 확인은 현재 데이터 조회나 비용 실증을 의미하지 않음 |
...
프로덕션 후보 이미지는 변경 가능한 태그가 아니라 `@sha256:<digest>`로 식별합니다.
목표는 private ACR에 접근하는 전용 runner를 이용한 반복 가능한 빌드이지만,
현재 대상 RG에는 runner VM이 없습니다. 지금까지의 배포는 명시적으로 승인된
일회성 로컬 빌드·push 후 임시 접근을 제거한 경로였으며 상시 공개 ACR을 뜻하지 않습니다.
현재 참조 배포 `7c1336bc`와 미배포 코드·문서 차이는 [현재 상태](docs/current-status.md#repository-only-gap)를 참고하십시오.
...
- [ ] 최종 후보의 Agent 365 원본이 최신 영속 스냅샷에서 `ready + complete`임을 재확인하고 관측 시각·package/agent-package/extension 수량을 기록
