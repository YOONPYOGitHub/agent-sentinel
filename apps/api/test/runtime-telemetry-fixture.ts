...
function observations(
  ...
  return Array.from({ length: 10 }, (_, index) => {
    ...
    return {
      ...
      correlations: [
        { kind: 'agent-run-id', value: `${evidencePrefix}-run` },
        { kind: 'correlation-id', value: `${evidencePrefix}-correlation` },
        { kind: 'agent-version', value: '17' },
      ],
      otelProvenance: {
        snapshotGeneratedAt: request.snapshotGeneratedAt ?? QUERIED_AT,
