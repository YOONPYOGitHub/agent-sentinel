...
Agent Sentinel 제어 계층의 문서 색인입니다. `feature/production-readiness-r1`의
코드 기준점 `1129bbe8`과 배포 `7c1336bc`를 **2026-09-15**에 대조했습니다.
변동 수량·실제 배포·미배포 기능·미완료 승인의 단일 기준은 [현재 상태](current-status.md)입니다.
...
- 문서가 한국어라는 이유만으로 현행이라고 판단하지 않습니다. 과거 테넌트 예시·검증 기록은 날짜를 유지하며, 현재 운영 명령과 구분합니다.
- Azure·메뉴·API 이름은 원어로 유지하고 설명은 한국어로 작성합니다. 오래된 링크의 heading anchor를 보존합니다.
