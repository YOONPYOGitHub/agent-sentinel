...
**Status date: 2026-08-28** · Branch: `feature/multi-source-otel`
...
| Multi-source OTel routing foundation       | **Implemented; activation pending.** Aggregate agent ids resolve through the persisted snapshot to exact source workspace/tenant/environment/provider-agent bindings. Results rebind to the aggregate agent, and per-source health is exposed.                                                                                                                   |
...
| Entra identity enrichment                    | Read-only multi-source Graph inventory, tenant/environment-scoped Foundry correlation, per-source health, API/jobs wiring, and packaging are implemented. `Application.Read.All` is granted to the primary UAMI; bounded validation and activation remain. |
...
| Runtime telemetry activation                 | Workspace-scoped `Log Analytics Reader` is granted and synthetic validation trace instrumentation is implemented. AppRequests delivery, bounded query validation, and connector activation remain.                                                         |
