...
| Entra identity enrichment                  | **Live for the replacement primary source.** `Application.Read.All` reads 315 validated service principals into the complete Cosmos snapshot after Defender onboarding. Optional owners, app roles, and preview APIs remain disabled. Current Foundry agents expose no matching identity metadata, so `RUNS_AS` correlation is correctly empty rather than inferred.                                                                                                               |
