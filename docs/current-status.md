...
| Universal custom manifest adapter          | **Implementation complete, activation pending.** Strict offline input, immutable ingestion, jobs composition, and exact source-bound runtime-claim verification are implemented. Only non-synthetic Azure Monitor evidence can verify a claim; no graph relationship is inferred. Live ingestion stays blocked by writes-false and the public mutation posture.  |
