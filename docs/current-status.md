...
| `1129bbe8727ab9cb7a2e49a1f417a8e042d9ad94` | Release readiness에서 기록되지 않은 설정 시각과 실제 관측 시각 구분 | 미배포; 전체 실행 결과와 이전 시간 초과는 아래 검증 기록 참조 |
...
문서 현행화는 제품·인수인계 10개, 설계·명세 7개, 운영 9개를 통합했습니다.
커넥터·SDK 12개는 별도 현행화 결과의 통합이 남아 있습니다.
기존 38개 문서의 한국어 정리 완료와 전체 사실관계 현행화 완료를 구분합니다.
...
- 이 관측의 Agent 365 패키지 309개는 agent-package 노드 303개와 extension-package control 6개입니다. 이전에 기록한 308개(302+6)는 그 당시의 정상 관측이며 새로운 수량으로 과거 증거를 덮어쓰지 않습니다.
...
**깨끗한 `df155b3d4ab7566607b2fe4945af645a2f75764b`에서 전체 순차 실행을 완료했습니다.**
build 27/27 task, lint·typecheck·test 각 52/52 task가 통과했습니다.
단위·계약 테스트는 2,046개 통과·1개 생략이며 Web 346개를 포함합니다.
Node 22.23.2와 pnpm 10.15.1에서 캐시 재사용 없이 build → lint → typecheck → test
순서로 실행했습니다. build/lint/typecheck의 Turbo concurrency는 2,
test의 Turbo concurrency와 각 Vitest worker 수는 1이었습니다.
이 결과는 해당 SHA의 실행 증거이지 Azure 배포·사람의 승인·최종 동결 완료가 아닙니다.
커넥터 문서 통합과 최종 배포 버전의 증거 갱신은 남아 있습니다.

**이전 실패와 환경 진단 기록은 보존합니다.** `1129bbe8`의 설정 시각 수정은 SDK 81개,
해당 Web 화면 4개, readiness CLI 8개 테스트와 관련 lint/typecheck 및 Web build를
통과했습니다. 그러나 전체 Web 실행은 기본 병렬 실행에서 8개, worker 2개 실행에서
서로 다른 5개, worker 1개 실행에서 2개 테스트가 5초 제한으로 시간 초과했습니다.
두 파일의 격리 실행에서도 77ms로 보고된 테스트가 5초 초과로 실패해, 단순한 화면
처리 지연으로 단정할 수 없습니다.

진단용 시계 관측을 추가한 별도 실행은 Web 346개가 통과했지만, 한 테스트에서
monotonic 경과 시간은 약 149ms인데 `Date.now()` 경과 값은 -1,120ms였습니다.
설치된 Vitest 3.2.7의 완료 시 timeout 판정은 `Date.now()` 차이를 사용합니다.
이는 실행 환경의 시계 불안정성을 보여 주지만 모든 이전 실패의 원인을 확정하지는
않습니다. 위 `df155b3d` 전체 실행은 임시 진단 코드를 제거한 뒤 수행했으며 assertion,
5초 제한, 테스트 프레임워크 및 시스템 시계 설정은 변경하지 않았습니다.
worker 1개 실행의 통과를 기본 병렬 실행의 간헐적 시간 초과가 해소된 증거로
일반화하지 않습니다. 이후 코드 변경은 새 후보 SHA의 결과로 구분해야 합니다.
