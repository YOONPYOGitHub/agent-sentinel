...
| Unified live finding model                   | Exposure and governance use persisted `ExposureFinding` records. The overview still derives its legacy attack-path `Finding` summary from the same persisted snapshot; consolidating those two finding shapes remains.              |
