...
| Connector management catalog               | **Current status surface.** Replacement-tenant Foundry, Entra, Purview, Teams organization catalog, Azure Resource Graph, and Azure Monitor OTel sources are connected. The manifest adapter remains activation-gated; Agent 365, Defender, Power Platform, and other platform entries retain their explicit prerequisite states.                                                                                                                                                  |
...
| Entra identity enrichment                  | **Live for the replacement primary source.** `Application.Read.All` reads 311 validated service principals into the complete Cosmos snapshot. Optional owners, app roles, and preview APIs remain disabled. Current Foundry agents expose no matching identity metadata, so `RUNS_AS` correlation is correctly empty rather than inferred.                                                                                                                                         |
...
| Defender for Cloud Apps evidence           | **Activation pending.** Replacement M365 E5 exposes the relevant Defender service plans, but the exact Defender for Cloud Apps tenant portal and `Investigation.Read` application permission are not configured. The connector remains disabled and makes no availability claim.                                                                                                                                                                                                   |
...
| Azure deployment                           | **Live in the replacement tenant.** Container Apps `web`, `api`, and `jobs` run on a private ACA environment behind Front Door. Foundry, Entra, Purview, Teams catalog, Azure Resource Graph, and Azure Monitor reads are active; authentication and every write path remain disabled.                                                                                                                                                                                             |
...
| Defender for Cloud Apps activation   | The replacement Defender workspace is provisioning; its exact API URL and `Investigation.Read` assignment are not yet available.             | Provisioning completes, then the exact About-page URL and separately approved read permission pass bounded probes. |
