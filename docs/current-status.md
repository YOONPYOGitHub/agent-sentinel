...
| Evidence-backed scorecard foundation       | **Partial live integration.** Agent-detail security uses Cosmos exposures. Governance mixes declared owner/trust with exposures; lifecycle also depends on process-local legacy validation runs. Cost, quality, and reliability remain `unknown` live.                                                                                                           |
...
| Unified live product read model              | Exposure and governance posture use jobs-generated Cosmos records. Inventory, catalog, lifecycle, and observability still use direct `/api/demo/state` discovery and process-local legacy findings/runs, so surfaces can disagree.  |
