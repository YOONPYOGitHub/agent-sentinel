...
| Teams tenant app catalog                   | **Implementation complete, activation pending.** Official Global Graph v1.0 organization catalog metadata is collected with strict bounds and sequential multi-tenant composition. Records remain control evidence with no agent, deployment, installation, sideloading, distribution coverage, trust, tool, entitlement, or access inference. No permission or live source was created. |
...
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unified inventory for future connectors      | Inventory renders any connector's records. Foundry supplies live data; Power Platform inventory is implemented but disabled until separately approved tenant-scope RBAC is available.              |
...
| Teams tenant app catalog activation  | Each source requires Global Graph access, a secretless credential, and tenant-admin application consent.                                          | Grant `AppCatalog.Read.All` separately, configure unique tenant sources, then validate catalog-only evidence.   |
...
- Live activation has **not** occurred. It remains `authorization-required`
  pending tenant-admin `AppCatalog.Read.All` application consent for every
  source. IaC adds no Graph app-role assignment or Microsoft 365 resource.
