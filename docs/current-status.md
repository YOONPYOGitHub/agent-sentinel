...
| Connector management catalog               | **Current status surface.** Replacement-tenant Foundry, Entra, Purview, Defender for Cloud Apps, Teams organization catalog, Azure Resource Graph, and Azure Monitor OTel sources are connected. Agent 365 entitlement, one assigned seat, approved UAMI `CopilotPackages.Read.All`, and bounded HTTP 200/306-package access are verified; its deployment-only source policy is implemented, while reviewed deployment and persisted snapshot validation remain pending. The manifest adapter remains activation-gated, while Power Platform and other platform entries retain their explicit prerequisite states. |
...
| Live role coverage                           | JWT/RBAC/MSAL behavior is implemented and tested in code. Replacement API/SPA registrations do not yet exist; approval/apply, consent, role assignments, reviewed image digests, deployment activation, and live-token validation remain pending.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Terra authenticated `POST`           | The active Front Door has no evidenced mutation rule; `BlockApiMutationPreAuth` exists only on the stopped Application Gateway. The API write switch remains false and write activation is prohibited.                                                                                                                                                                                                             | Authenticated write scopes are validated, then the rule is narrowed.                                                                                                        |
| Front Door write-edge rule           | An exact active Front Door mutation rule must be reviewed, deployed, and rediscovered together with JWT before any write-stage readiness.                                                                                                                                                                                                                                                                          | JWT write-scope tests and an authorized remediation smoke test pass.                                                                                                        |
...
| Agent 365 package activation         | `AGENT_365` entitlement is verified with one assigned seat, the exact approved connector UAMI has `CopilotPackages.Read.All`, and a bounded managed-identity list call returned HTTP 200 with 306 packages. Only deployment-origin sources using that UAMI can activate; user-origin and invalid legacy records remain visible but inactive. Reviewed deployment and persisted snapshot validation remain pending. | Independently review and deploy the focused image, then verify source-linked jobs/API health and classified persisted inventory without treating every package as an agent. |
...
- Agent 365 is deployment-managed only. API/jobs activate only deployment-origin sources using exact approved connector UAMI `59dbea72-1e91-403a-89cf-e02cdb8da350`; API and web mutation paths reject user-origin Agent 365 sources, while legacy invalid records remain visible but inactive with typed reasons. The reviewed image has not been deployed and no source-linked persisted snapshot has been validated. The 306 packages are provider catalog records, not a verified count of 306 agents.
...
- Retained package snapshots are projected through exact current source health on every API snapshot read. Only `ready` plus `complete` health bound to the same persisted snapshot generation and canonical evidence digest remains `live`; legacy unbound ready health cannot promote retained evidence, while unbound failure or degraded health can still demote it. Jobs save the final snapshot before bound success health and stop before finding publication if that health write fails.
- Deployment source IDs preserve an immutable binding to their original configured IDs, including changed or expired health projection. Agent 365 response reads race the combined caller/timeout abort signal, response cleanup is non-blocking, and caller cancellation or timeout is retained instead of being synthesized as authorization-required or a generic network/status failure.
- IaC passes `AGENT365_MANAGED_IDENTITY_CLIENT_ID` to API/jobs only and grants no Microsoft Graph app role or license; the verified entitlement and UAMI permission remain external deployment prerequisites.
