...
| Token Economics foundation                 | **Current.** `analyzeTokenEconomics()` uses measured-only populations, reconciled coverage, evidence-linked MAD anomalies, and same-population cost per success. API reports attach source-cited owner and business-unit values only from the exact authoritative agent; absent or non-authoritative values remain typed `partial` or `unknown`. Live mode accepts validated Azure Monitor OTel windows and remains `insufficient-data` below the ten-sample floor.                |
