...
- **@agent-sentinel/azure-monitor-otel-connector** - Read-only Azure Monitor Logs query adapter that strictly maps OTel `AppRequests` rows into tenant/agent/environment/time-bound `ObservationWindow` objects. Its provider-call-free representative normalizer also accepts bounded trace/span/metric pages, preserves exact estate/source/resource/trace/span provenance and sampling/aggregation caveats, and exposes only complete, fresh, unsampled, exact-ID groups to analysis. It performs no ingestion or Azure resource mutation.
