...
- **@agent-sentinel/azure-monitor-otel-connector** - Read-only Azure Monitor Logs query adapter that filters bounded OTel `AppRequests` rows by exact tenant, Foundry project, provider agent, and environment before producing time-bound `ObservationWindow` objects. It canonicalizes the complete response by observation ID before window partitioning, preserves exact snapshot-generation, estate, source-project, workspace, provider-agent, trace, span, runtime-correlation, version, and matched-tool provenance, and exposes only complete, fresh, unsampled, explicitly non-synthetic groups to live analysis. It performs no ingestion or Azure resource mutation and never substitutes `AppMetrics` or fixtures.
