...
코드·배포 경계 현행화: **2026-09-15**. 날짜별 수량과 배포 버전은 [현재 상태](current-status.md)가 기준입니다.
...
Agent Sentinel은 하나의 증거 집합으로 이 질문을 조사하도록 돕습니다. 현재는
읽기 전용 인벤토리와 선언 기반 정책·증거 연결을 시연할 수 있으며, 로그인·실행 신원·
적격 runtime telemetry·실제 remediation이 모두 완성된 서비스는 아닙니다.
...
flowchart LR
    ...
    R --> A["승인 + 감사되는 실행"]
    A -.writeEnabled=false로 차단, WAF guard 별도 승인.-> A
```
...
| 현재 미충족 사항 | 정확한 실행 신원 연결 미완료           | 수량은 [관측 원장](current-status.md#connector-ledger) 참조, 개인 접근 권한은 입증하지 않음 | 인증 비활성 및 authoritative entitlement resolver 미구성. 로그인만으로 해결되지 않음     | 적격 실제 OTel 증거 없음                                   |
...
Agent 365는 자신이 관리하는 에이전트의 권위 있는 레지스트리이자 관리 계층입니다. Sentinel은
`m365-agent-registry`로 표시되는 읽기 전용 **소비자**이며 소유권 모델은 `consumes`입니다.
패키지는 agent-package와 extension-package로 구분하고 실제 수량은 [관측 기록](current-status.md#connector-ledger)에 둡니다.
...
**Foundry의 6개 에이전트는 합성 검증용**이며 GPT-5.6 Terra를 사용하는 선언된 설정입니다.
현재 전체 스냅샷에는 실제 Agent 365 카탈로그 및 Entra 등 공급자 메타데이터도 있으므로
전체 데이터를 합성 전용이라고 부르지 않습니다. 아래는 설계된 검증 시나리오이며
이번 수집에서 agent를 호출·실행했다는 증거는 아닙니다.
...
읽기 전용 `azure-monitor-otel`은 구성되어 있지만 현재 적격 에이전트 0개로
`not-queried` 상태입니다. 외부 agent 실행 경계에 runtime SDK를 도입하고 정확한
원본 바인딩·계측·실측 데이터를 확보해야 합니다. SDK 자체가 실행이나 비용을
만들지 않으며 권위 있는 비용이 없으면 생략하고 현재 token/cost gate는 미충족으로 둡니다.
이 선행 조건이 제공되기 전까지는 다음과 같습니다.
...
| **범용 어댑터 실제 활성화** | 매니페스트 스키마·오프라인 검사·수집 API는 이미 구현됨. 실제 수집만 미활성                     | 인증·Administrator 역할·write scope·배포 쓰기·에지 승인 |
| **시프트 레프트 활용 확대** | `pnpm manifest:scan`은 이미 구현된 오프라인 기능. 신규 구현 대기가 아닌 CI 사용·증거 확보 과제 | 적절한 매니페스트 입력과 실행 결과 기록                 |
