...
| Azure Resource Graph                        | Complete, multi-source | **Live contract validated; deployment activation pending** | The fixed query succeeds with an authorized user credential. The application UAMI has narrower resource-scoped roles, so its actual visible coverage must be measured after deployment. It does not require M365 E5. | Deploy the connector with existing UAMI rights and measure coverage. Add broader Reader scope only after separate approval.            | Fixed-query Azure AI and supporting-resource inventory, provenance, multi-subscription boundaries, and source health. Never infer that a resource is an agent. |
| Azure Monitor and OpenTelemetry             | Complete, multi-source | **Telemetry-insufficient**                                 | The workspace and read-only query role exist, but current measured windows have sparse or no qualifying samples.                                                                                                     | Target agents emit the documented attributes and each evaluated window meets the minimum sample threshold.                             | Instrumentation, benign synthetic telemetry, freshness/coverage UX, drift, reliability, latency, and measured-only token economics.                            |
