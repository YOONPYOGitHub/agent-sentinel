# Connector availability

Status date: **2026-09-04**.

This ledger separates implementation state from live availability. A connector
is not described as connected merely because its code exists or an application
permission was assigned.

Latest live revalidation: **2026-09-01 14:23 KST**. Excluding the intentionally
deferred Agent 365 path, all seven configured sources are `ready` with no
partial failure. The remaining catalog entries are not missed connections:
Power Platform lacks supported unattended authorization, manifest ingestion is
write/auth-gated, the business-outcome source has no authoritative provider,
and MCP gateway telemetry has no selected provider contract.

This remains a historical sanitized observation until the integration owner
supplies a newer result. Connector readiness does not establish deployment
image parity, user authentication, exact agent-to-identity correlation, or
remediation execution. The 2026-09-01 Entra result is inventory only; no
`RUNS_AS` edge exists without one exact identifier match.

## State definitions

| State                         | Meaning                                                                                                      |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Connected                     | A bounded live read succeeded and its evidence can be composed into the estate snapshot.                     |
| Configured, provider-blocked  | Code, credentials, and intended read permission exist, but the provider rejects or cannot serve the request. |
| Licensing-blocked             | The provider workload or API is unavailable until the tenant receives the prerequisite product license.      |
| Approval-required             | A new permission, role, credential, or write-path change needs explicit review before activation.            |
| Telemetry-insufficient        | The source is queryable, but measured samples do not meet the evidence threshold.                            |
| Implemented, activation-gated | Code is complete, but the deployed safety posture intentionally prevents live ingestion.                     |
| Not implemented               | No production connector exists; mock or manifest evidence must remain explicitly non-live.                   |

## Current connector matrix

| Connector                                   | Implementation         | Live state in the current tenant                          | Why it is limited                                                                                                                                                                                                                                                       | Becomes available when                                                                                                                 | Development allowed now                                                                                                                                        |
| ------------------------------------------- | ---------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Azure AI Foundry Agent Service              | Complete, multi-source | **Connected**                                             | One live Foundry project is configured; additional projects need their own authorization boundary.                                                                                                                                                                      | Add an approved source credential and Azure AI User access for each project.                                                           | Discovery, strict contract validation, multi-source aggregation, source health, and declared tool metadata.                                                    |
| Microsoft Entra service-principal inventory | Complete, multi-source | **Inventory connected; parity unmatched**                 | Stable v1.0 inventory is live with 335 identity nodes. The 6 authoritative Foundry agents expose null instance identity and null blueprint reference, so exact correlation currently emits 0 `RUNS_AS` edges. Owner/app-role reads and preview Agent Identity classification are independent capabilities; disabled or failed reads are not zero coverage. | Foundry exposes one exact source-local object ID or app/client ID, or preview is separately approved and confirms an Agent Identity ID. | Exact per-source diagnostics, bounded owner/app-role enrichment, evidence references, and no-match/ambiguity UX. Never correlate by name, alias, owner, or fuzzy text. |
| Azure Resource Graph                        | Complete, multi-source | **Connected for the current authorized view**             | The application UAMI query is live and persisted 5 visible resources. Its existing resource-scoped roles do not prove full subscription coverage. It does not require M365 E5.                                                                                          | Keep the current least-privilege view, or separately approve broader Reader scope if complete subscription inventory is required.      | Fixed-query Azure AI and supporting-resource inventory, provenance, multi-subscription boundaries, and source health. Never infer that a resource is an agent. |
| Azure Monitor and OpenTelemetry             | Complete, multi-source | **Connected query path; telemetry-insufficient analysis** | The replacement workspace query is live, but measured populations must independently meet the ten-sample evaluation floor. Representative local trace/span/metric normalization now preserves exact resource and span provenance and fails closed on sampling, aggregation, partiality, staleness, unsupported records, or incomplete pages. Local fixtures do not establish live readiness. Direct run IDs, operation correlation IDs, and version context are preserved; only run/correlation IDs count as exact outcome-link coverage. | Collect complete, fresh, unsampled representative measured traffic across both baseline and observed windows and revalidate the provider mapping in the replacement tenant. | Instrumentation, benign synthetic telemetry, freshness/coverage UX, drift, reliability, latency, and measured-only token economics.                            |
| Business outcome source                     | Contract complete      | **Not configured; value remains unknown**                 | No authoritative outcome system currently supplies an exact accepted run, correlation, or discovered agent-version identifier. Mock outcomes remain explicitly synthetic.                                                                                               | Configure a supported read-only outcome source with exact correlation and direct observed evidence.                                    | Strict contracts, fail-closed API/UI, synthetic fixtures, and connector catalog status. No aggregation, monetary estimates, or invocation proxies.             |
| Microsoft Purview sensitivity-label catalog | Complete, multi-source | **Connected**                                             | The replacement source persists 12 bounded label controls. Catalog definitions do not prove label use, content protection, user activity, agent attribution, trust, or compliance.                                                                                      | A separately reviewed supported usage source supplies those facts.                                                                     | Label-definition evidence, pagination/contract safety, provenance, and catalog health.                                                                         |
| Power Platform ResourceQuery                | Complete, multi-source | **Unsupported for unattended activation**                 | Microsoft currently documents delegated `ResourceQuery.Resources.Read` plus an Entra role for inventory. Preview Power Platform RBAC roles are explicitly unsupported for inventory, and an environment query filter is not an authorization boundary.                  | Microsoft documents a production-supported app-only inventory permission with an enforceable intended scope.                           | Schema fixtures, bounded paging, sanitized failure reasons, retry behavior, and partial-source composition. Do not report inventory as empty or healthy.       |
| Microsoft Agent 365 package catalog         | Complete, multi-source | **Licensing-blocked; deferred**                           | Replacement M365 E5 and Teams are active, but the Agent 365 product license and `CopilotPackages.Read.All` are not assigned. Agent 365 is optional for core runtime and is intentionally deferred until the supported MCAPS acquisition route is resolved.              | Provision Agent 365 and approve the exact read-only Graph permission.                                                                  | Package mapping, M365/SharePoint coverage, contract-drift handling, catalog UX, and tests. No live success claim.                                              |
| Microsoft 365 and SharePoint agents         | Covered by Agent 365   | **Licensing-blocked**                                     | These packages use the Agent 365 catalog. There is no separate approved SharePoint scraping connector.                                                                                                                                                                  | Agent 365 prerequisites above are satisfied.                                                                                           | Package-type classification and entitlement-safe UX without reading SharePoint content.                                                                        |
| Microsoft Teams organization app catalog    | Complete, multi-source | **Connected**                                             | Teams Enterprise and `AppCatalog.Read.All` are active in the replacement tenant. The current bounded organization-catalog result contains zero entries and makes no installation, distribution, trust, or coverage claim.                                               | No additional prerequisite for the current catalog-only read.                                                                          | Bounded organization-catalog mapping, provenance, pagination, and tests. Do not enumerate teams, chats, users, groups, or installations.                       |
| Microsoft Defender for Cloud Apps           | Complete, multi-source | **Connected**                                             | Defender XDR is provisioned, the exact tenant API URL and `Investigation.Read` are active, and API/jobs bounded probes are ready. The new tenant currently returns zero alerts and activities, so no Defender control/evidence nodes are persisted.                     | No additional prerequisite for the current read-only alert/activity lists.                                                             | Privacy-reduced alert/activity contracts, unattributed evidence, failure handling, and tests. Do not infer agent joins.                                        |
| Custom manifest adapter                     | Complete               | **Implemented, activation-gated**                         | Offline validation and scanning work. Live API ingestion is blocked by writes-false and the public pre-auth mutation WAF rule.                                                                                                                                          | Administrator role and write scope pass live validation, the private write smoke test succeeds, and the WAF rule is narrowly reviewed. | Offline manifests, third-party examples, first-party reconciliation, validation, and shift-left scanning.                                                      |
| MCP gateway telemetry                       | Not implemented        | **Not implemented**                                       | No approved authoritative gateway source and contract is configured.                                                                                                                                                                                                    | Select a real gateway, document its supported telemetry API/schema, and approve the read boundary.                                     | Domain contract and fixtures only after a real provider contract is selected.                                                                                  |

## Tenant transition impact

- The replacement Managed Environment is provisioned and hosts the active Azure,
  Foundry, Entra, Purview, Defender for Cloud Apps, Teams catalog,
  application, and telemetry validation paths.
- The historical tenant is scheduled for retirement and is no longer an
  activation target.
- Replacement Microsoft 365 E5 (no Teams) and Teams Enterprise licensing are
  active.
- M365 E5 alone does not automatically prove every connector available:
  - Agent 365 also needs the Agent 365 product license and Graph consent.
  - Teams catalog read is connected through the separately licensed Teams
    Enterprise backend.
  - Defender for Cloud Apps is connected through the provisioned Defender XDR
    tenant and its separately approved application permission.
  - Power Platform inventory lacks a supported unattended authorization path;
    licensing alone does not unblock it.
- Exact request, tenant, subscription, and support identifiers are kept in the
  git-ignored local handoff, not this repository document.

## Activation rules

1. Keep every new connector disabled by default.
2. Use only documented read APIs and least-privilege application or Azure RBAC.
3. Validate tenant, subscription, environment, and source boundaries on every response.
4. Fail closed on malformed or truncated provider data.
5. Preserve provider failure as typed health; never replace it with mock success.
6. Map direct cloud resources as control/evidence unless an authoritative API identifies an agent.
7. Require separate approval before adding permissions, widening RBAC, enabling writes, or changing the WAF.
