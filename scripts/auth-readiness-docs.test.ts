...
describe('authentication readiness documentation', () => {
  ...
  it('documents approval, all four roles, and immutable deployment evidence as blockers', () => {
    ...
    const activation = section(security, 'activation-checklist', 'emergency-rollback')
    expect(activation).toMatch(/대체 API 및 SPA 앱 등록과 service principal 존재 확인/)
    expect(activation).toMatch(/기존 대체 API Read\/Write scope[^\n]*승인된 계획에 대조/)
    expect(activation).toMatch(/- \[ \] 검토된 이미지 다이제스트[^\n]*`AUTH_MODE=jwt` 배포/)
    expect(security).toMatch(/4개 역할 모두/)
    expect(handoff).toMatch(/Viewer[\s\S]*Analyst[\s\S]*Approver[\s\S]*Administrator/)
    expect(knownIssues).toMatch(/승인된 전체 SHA[^\n]*배포/)
